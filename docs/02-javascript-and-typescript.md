...
step. It is everywhere in this codebase:

```ts
const { baseUrl } = useConfig()                  // src/App.tsx:35  — grab one field
const { loading, error, data } = useDataQuery(…) // src/App.tsx:36  — grab several
const PartnerReportForm = ({ onSubmit, submitting = false }) => { … }  // with a default
```

The `submitting = false` gives a **default value** used when the caller omits it.

The **spread** operator `...` copies fields. This pattern — copy the old object,
...
variable rather than being typed literally.

The `...init` in `src/api/neoipcReporting.ts:42` does the same for the fetch
options — start from a default object, then let the caller's `init` override.
